  ...
  <div class="col-12">
   <div class="card">
    <div class="card-header">
     <h4 class="card-title">
      <a href="{{ route('home') }}" class="btn btn-success btn-round">Back To Home</a>
     </h4>
    ...
    <div class="card-content collapse show">
     <div class="card-body card-dashboard">
      <p class="card-text">User Log Activities
      </p>
      ...
      <table class="table table-striped table-bordered dataex-res-configuration">
       ...
       <tbody>
        ...
        <tr>
         ...
         <td>{{ $log->ip }}</td>
         <td>{{ $log->agent }}</td>
         <td>Account ID: {{ $log->user_id }}
          <br>
          <small> {{ $log->user->name }} </small>
         </td>
         <td>
          <a href="{{ route('admin.logActivity.show', $log->id) }}" class="btn btn-primary btn-sm">Show</a>
          <form action="{{ route('admin.logActivity.destroy', $log->id) }}" method="POST">
           @csrf
           @method('DELETE')
